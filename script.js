...
/* ========== TIMELINE SECTION (JQUERY) ========== */

function initTimeline() {
    // Проверка наличия jQuery
    if (typeof jQuery === 'undefined') {
        console.warn('jQuery не загружен. Timeline функциональность недоступна.');
        return;
    }

    // Фильтрация элементов Timeline
    $('#timelineFilters').on('click', '.timeline-filter-btn', function() {
        const $btn = $(this);
        ...
        // Фильтруем элементы
        if (filter === 'all') {
            $('.timeline-item').removeClass('hidden').fadeIn(400);
        } else {
            $('.timeline-item').each(function() {
                ...
                if (category === filter) {
                    $item.removeClass('hidden').fadeIn(400);
                } else {
    ...
    // Инициализируем начальное состояние элементов
    $('.timeline-item').css({
        'opacity': '0',
    ...
    // Клик по карточкам для раскрытия дополнительной информации (опционально)
    $('.timeline-card').on('click', function() {
        const $card = $(this);
        const $description = $card.find('.timeline-description');
        
        // Переключение класса для расширенного просмотра
    ...
    // Статистика с анимацией чисел
    function animateTimelineStats() {
        $('.timeline-stat-number').each(function() {
            const $stat = $(this);
            const targetValue = parseInt($stat.data('count'));
            const currentText = $stat.text();
            
            // Проверяем, не анимировалось ли уже
            if ($stat.hasClass('animated')) return;
            ...
            $({ count: 0 }).animate(
                { count: targetValue },
                {
                    ...
                    step: function() {
                        ...
                        $stat.text(Math.floor(this.count) + suffix);
                    },
                    complete: function() {
                        $stat.text(currentText);
                    }
                }
            );
        });
    ...
    $(window).on('scroll', function() {
        ...
        if (scrollTop + windowHeight > statsTop + 100 && !$stats.hasClass('stats-animated')) {
            $stats.addClass('stats-animated');
            animateTimelineStats();
        }
    });

    // Плавная прокрутка к Timeline при клике на навигацию
    ...
    console.log('Timeline инициализирован с jQuery');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Инициализация Timeline с jQuery (после загрузки jQuery)
    if (typeof jQuery !== 'undefined') {
        initTimeline();
    }
});
